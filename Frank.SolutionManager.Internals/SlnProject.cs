...
internal sealed class SlnProject
{
    ...
    /// </summary>
    internal static readonly Guid DefaultLegacyProjectTypeGuid = new (VisualStudioProjectTypeGuids.LegacyCSharpProject);

    /// <summary>
    /// Represents the default project type GUID for projects that load in the NetSdk project system.
    /// </summary>
    internal static readonly Guid DefaultNetSdkProjectTypeGuid = new (VisualStudioProjectTypeGuids.NetSdkCSharpProject);

    /// <summary>
    /// Known project type GUIDs for legacy projects.
    /// </summary>
    internal static readonly IReadOnlyDictionary<string, Guid> KnownLegacyProjectTypeGuids = new Dictionary<string, Guid>(StringComparer.OrdinalIgnoreCase)
    {
    ...
    /// </summary>
    internal static readonly IReadOnlyDictionary<string, Guid> KnownNetSdkProjectTypeGuids = new Dictionary<string, Guid>(StringComparer.OrdinalIgnoreCase)
    {
    ...
    /// </summary>
    internal static readonly IReadOnlyDictionary<string, Guid> KnownProjectTypeGuids = new Dictionary<string, Guid>(StringComparer.OrdinalIgnoreCase)
    {
    ...
    /// </summary>
    internal IReadOnlyList<string> Configurations { get; set; } = Array.Empty<string>();
    ...
    /// </summary>
    internal IReadOnlyList<string> Platforms { get; set; } = Array.Empty<string>();
    ...
    /// </summary>
    internal Guid ProjectTypeGuid { get; set; }

    /// <summary>
    /// Gets or sets a list of shared project items.
    /// </summary>
    internal IReadOnlyList<string> SharedProjectItems { get; set; } = Array.Empty<string>();

    /// <summary>
    /// Gets or sets the name of a solution folder to place the project in.
    /// </summary>
    internal string SolutionFolder { get; set; }
    ...
    internal static SlnProject FromProject(Project project, IReadOnlyDictionary<string, Guid> customProjectTypeGuids, bool isMainProject = false)
    {
        if (project == null)
        {
    ...
    internal static IEnumerable<string> GetSolutionItems(Project project, ILogger logger, Func<string, bool> fileExists = null)
    {
        if (project == null)
        {
            throw new ArgumentNullException(nameof(project));
        ...
        foreach (string solutionItem in project.GetItems(MSBuildItemNames.SlnGenSolutionItem).Select(i => i.GetMetadataValue("FullPath")).Where(i => !i.IsNullOrWhiteSpace()))
        {
            ...
            {
                logger.LogInformation("The solution item \"{solutionItem}\" does not exist and will not be added to the solution.", MessageImportance.Low);
            }
            else
            {
