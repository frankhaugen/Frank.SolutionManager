...
internal sealed class SlnFile
{
    ...
    internal SlnFile(string fileFormatVersion)
    {
    ...
    /// </summary>
    internal IReadOnlyCollection<string> Configurations { get; set; }

    /// <summary>
    /// Gets or sets a <see cref="IReadOnlyDictionary{TKey,TValue}" /> containing any existing project GUIDs to re-use.
    /// </summary>
    internal IReadOnlyDictionary<string, Guid> ExistingProjectGuids { get; set; }

    /// <summary>
    /// Gets or sets an optional minimum Visual Studio version for the solution file.
    /// </summary>
    internal string MinimumVisualStudioVersion { get; set; } = "10.0.40219.1";

    /// <summary>
    /// Gets or sets a <see cref="IReadOnlyCollection{String}" /> of Platform values to use.
    /// </summary>
    internal IReadOnlyCollection<string> Platforms { get; set; }

    /// <summary>
    /// Gets or sets a <see cref="Guid" /> for the solution file.
    /// </summary>
    internal Guid SolutionGuid { get; set; } = Guid.NewGuid();

    /// <summary>
    /// Gets a list of solution items.
    /// </summary>
    internal IReadOnlyDictionary<string, IReadOnlyCollection<string>> SolutionItems => _solutionItems.ToDictionary(
        k => k.Key,
    ...
    /// </summary>
    internal Version VisualStudioVersion { get; set; }
    ...
    internal static (string solutionFileFullPath, int customProjectTypeGuidCount, int solutionItemCount, Guid solutionGuid) GenerateSolutionFile(ProgramArguments arguments, IEnumerable<Project> projects, ILogger logger)
    {
        ...
        logger.LogError($"Generating Visual Studio solution \"{Path.GetFullPath(solutionFileFullPath)}\" ...");

        if (customProjectTypeGuids.Count > 0)
        {
            logger.LogInformation("Custom Project Type GUIDs:");
        ...
        {
            logger.LogInformation("Updating existing solution file and reusing Visual Studio cache");
    ...
    internal static bool TryParseExistingSolution(string path, out Guid solutionGuid, out IReadOnlyDictionary<string, Guid> projectGuidsByPath)
    {
    ...
    internal void AddProjects(IEnumerable<SlnProject> projects)
    {
    ...
    internal void AddProjects(IEnumerable<Project> projects, IReadOnlyDictionary<string, Guid> customProjectTypeGuids, string mainProjectFullPath = null)
    {
        _projects.AddRange(
            projects
    ...
    internal void AddSolutionItems(IEnumerable<string> items)
    {
    ...
    /// <param name="items">An <see cref="IEnumerable{String}"/> containing items to add to the solution.</param>
    internal void AddSolutionItems(string folderPath, IEnumerable<string> items)
    {
    ...
    /// <param name="items">An <see cref="IEnumerable{String}"/> containing items to add to the solution.</param>
    internal void AddSolutionItems(string folderPath, Guid folderGuid, IEnumerable<string> items)
    {
    ...
    /// <param name="folderGuid">The unique GUID for the folder.</param>
    /// <param name="items">An <see cref="IEnumerable{String}"/> containing items to add to the solution.</param>
    internal void AddSolutionItems(Guid? parentFolderGuid, string folderPath, Guid folderGuid, IEnumerable<string> items)
    {
    ...
    internal void Save(string path, bool useFolders, ILogger logger = null, bool collapseFolders = false, bool alwaysBuild = true)
    {
    ...
    /// <param name="collapseFolders">An optional value indicating whether or not folders containing a single item should be collapsed into their parent folder.</param>
    /// <param name="alwaysBuild">An optional value indicating whether or not to always include the project in the build even if it has no matching configuration.</param>
    internal void Save(string rootPath, TextWriter writer, bool useFolders, ILogger logger = null, bool collapseFolders = false, bool alwaysBuild = true)
    {
    ...
    public void AddSolutionItems(SlnFolder parentFolder)
    {
        foreach (var slnProject in parentFolder.Projects)
        {
            _projects.Add(slnProject);
        }

        foreach (var slnFolder in parentFolder.Folders)
        {
            AddSolutionItems(slnFolder);
        }
    }
}